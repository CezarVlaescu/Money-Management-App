...
export class LocalDeletionTombstoneService {
  public readonly tombstones: WritableSignal<LocalDeletionTombstone[]> = signal<LocalDeletionTombstone[]>(this.loadTombstones());

  public add(entityType: DeletedEntityType, localId: string): void {
    const nextTombstones = [
      ...this.tombstones().filter(
        tombstone =>
          !(tombstone.entityType === entityType && tombstone.localId === localId)
      ),
  ...
  public remove(entityType: DeletedEntityType, localIds: string[]): void {
    ...
    const nextTombstones = this.tombstones().filter(
      tombstone =>
        tombstone.entityType !== entityType || !localIdsSet.has(tombstone.localId)
    );
  ...
  private loadTombstones(): LocalDeletionTombstone[] {
    ...
    if (!rawValue) return [];
    try { return JSON.parse(rawValue) as LocalDeletionTombstone[]; } 
    catch { return []; }
  }
}
