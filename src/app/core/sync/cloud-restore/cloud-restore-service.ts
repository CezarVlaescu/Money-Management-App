...
export class CloudRestoreService {
  ...
  public async restoreCloudDataToLocal(): Promise<CloudRestoreResult> {
    try {
      ...
      this.expensesService.replaceExpenses(
        expenses.map((expense) => this.mapCloudExpenseToLocalExpense(expense)),
        { skipAutoSync: true },
      );
      this.savingsGoalsService.replaceGoals(
        goals.map((goal) => this.mapCloudGoalToLocalGoal(goal)),
        { skipAutoSync: true },
      );
  ...
  private restoreSettings(settings: CloudUserSettings): void {
    ...
    if (settings.theme === 'dark' && !this.themeService.isDarkMode())
      this.themeService.toggleTheme();
    if (settings.theme === 'light' && this.themeService.isDarkMode())
      this.themeService.toggleTheme();
  }
